...
<div class="flex justify-center">
    <div class="w-6/12 bg-white p-6 rounded-lg  mt-5 mb-5">
        ...
        <form action="{{ route('posts.update', ['id' => $post->id]) }}" method="post" class="mb-4"> 
            ...
            @method('PUT')
            <div class="mb-4">
                <textarea name="body" id="body" cols="30" rows="4" class="bg-gray-100 border-2 w-full p-2 rounded-lg text-left @error('body') border-red-500 @enderror">{{ old('body', $post->body) }}</textarea>
                @error('body')
